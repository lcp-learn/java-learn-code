package com.lcp.learn.base.collections.utils;

/**
 * desc:    <br/>
 *
 * @author lichunpeng
 * @since 2019-07-16-17:40
 */
public class SnowFlakeGenerator {
  ...
  public synchronized long nextId() {
    ...
    if (currentStamp < lastStamp) {
      throw new RuntimeException(String.format("Clock moved backwards. Refusing to generate id for %d " +
          "milliseconds", lastStamp - currentStamp));
    }
    ...
    return (currentStamp - START_STAMP) << timestampBitLeftOffset
        | idcId << idcBitLeftOffset
        | machineId << machineBitLeftOffset
        | sequence;
  }
  ...
  public static class Factory {
     ...
     * 每一部分占用位数的默认值
     */
    private static final int DEFAULT_MACHINE_BIT_NUM = 5;//机器标识占用的位数
    private static final int DEFAULT_IDC_BIT_NUM = 5;//数据中心占用的位数
